...
## 2. 파츠별 개별 앵커 적용 (Per-Part Custom Anchors & Socket Alignment)
가슴 메인 앵커 외에도 각 파츠(머리, 오른팔, 왼팔, 다리)가 자신만의 결합부 소켓 앵커 오프셋(`rightPivot`, `leftPivot`, `pivotX`, `pivotY`, `offsetX`, `offsetY`)을 정의할 수 있도록 파츠 수준의 개별 앵커 시스템을 수립하였습니다.

- **독립 소켓 피벗 정렬**: 파츠별로 원본 이미지 해상도나 관절 구형 축 위치가 다르더라도, 각 파츠의 고유 피벗(`pivotX`, `pivotY`) 및 개별 마운팅 오프셋(`offsetX`, `offsetY`)을 가슴 앵커 좌표계에 1:1로 맞춤 결합.
- **간섭 없는 파츠 장착**: 특정 파츠(예: `arm_mech_laser`)에 개별 앵커가 부여되더라도 기본 파츠(`arm_red_robot`)의 마운팅 좌표나 위치에는 아무런 간섭이나 영향을 주지 않도록 완벽히 격리 연동.

---

## 3. 동적 기동 방식 스위처 (`recalculateAnchors`)
...
## 5. 파츠별 독립 오프셋 & 피벗 제어 시스템 (`customSize` / `offsetX`, `offsetY`)
...
## 6. 렌더러 메모리 자산 캐싱 (`ImageCache`) & 동기식 즉시 재드로잉
...
## 7. 최상단 Z-Index 덮어쓰기 파티클 렌더링 (Post-Image Rendering)
...
## 9. 실시간 시스템 & 파츠 디버그 로그 콘솔 (`#debug-log-console`)
